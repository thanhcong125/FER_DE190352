    ...
    <Carousel interval={3000} data-bs-theme="dark">
      {carouselMovies.map((m) => ( 
        <Carousel.Item key={m.id}> 
          <img className="d-block w-100" src={m.poster} alt={m.title} style={{ height: 420, objectFit: "contain" }} /> 
          <Carousel.Caption className="text-start" style={{ background: "rgba(0,0,0,0.35)", borderRadius: 12, padding: "0.75rem 1rem", maxWidth: 720 }}> 
            <h3 className="mb-1">
              {m.title}{" "} 
